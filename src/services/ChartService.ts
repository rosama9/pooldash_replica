    ...
    static loadChartData = (dateRange: DateRange, pool: Pool): ChartCardViewModel[] => {
        ...
        entries.forEach((entry) => {
            entry.readingEntries.forEach((reading) => {
                ...
                // Special case because salt charts for reading & treatment are hard to disambiguate:
                if (reading.id === 'salt') {
                    graphable.title = 'Salt Reading';
                }
                ...
                if (
                    ...
                    const i = idsToGraph.findIndex((g) => g.id === graphable.id);
            ...
            entry.treatmentEntries.forEach((treatment) => {
                ...
                // Special case because salt charts for reading & treatment are hard to disambiguate:
                if (treatment.id === 'salt') {
                    graphable.title = 'Salt Treatment';
                }
        ...
        return idsToGraph.map((graphable) => {
            ...
            entries.forEach((entry) => {
                entry.readingEntries.forEach((reading) => {
                    if (`${reading.id}|r` === graphable.id) {
                        dates.push(entry.userTS);
                        values.push(reading.value);
                    }
                });
                entry.treatmentEntries.forEach((treatment) => {
                    if (`${treatment.id}|t` === graphable.id) {
                        dates.push(entry.userTS);
