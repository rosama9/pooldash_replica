export class Util {
    ...
    static safeToFixed = (reading: { defaultValue: number, decimalPlaces: number }, value?: number): string => {
        const { defaultValue, decimalPlaces } = reading;
        const max = Math.max(decimalPlaces, 0);

        if (value !== undefined) {
          return value.toFixed(max);
        } else {
          return defaultValue.toFixed(max);
        }
      }
}
